...
pub struct Block {
    ...
    pub locked: bool,
    /// **Arbitrary block properties** — every frontmatter `key: value` that isn't one mkb manages
    ...
    /// not represented here, and `set_props` can only write scalars. mkb imposes no schema on
