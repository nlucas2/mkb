...
//!
//! A block file is **clean Markdown** with optional YAML frontmatter. mkb only reads a few
...
/// when there is metadata mkb manages: a `title:`, `tags:`, the `locked:` flag, an `updated:`
...
pub fn write_block(
    ...
    let clean_props: Vec<(&str, &str)> = props
        ...
        // `created`/`updated`) would collide with mkb's own metadata — e.g. a smuggled
...
/// the SPEC promise that a raw vault is recoverable with or without mkb.
...
/// The frontmatter keys mkb manages directly; everything else becomes a block property.
...
/// Whether `key` (case-insensitively) is one mkb manages itself (`title`/`tags`/`locked`). Such
...
/// key isn't one mkb manages (`title`/`tags`/`locked`). Only simple scalar values are captured
...
/// every key mkb will write, it will read back identically.
