...
impl<I: Index> SyncEngine<I> {
    ...
    /// outside `assets/` is refused, so this can only remove files mkb manages as assets.
    ...
    pub fn set_props(
        ...
        for (k, v) in props {
            ...
            // Reject keys mkb manages itself — properties must not shadow title/tags/locked
            // (locked is a human-only flag agents cannot set via the property path).
            if crate::blockfile::is_managed_key(k) {
                return Err(IndexError::new(format!(
                    "reserved property key {k:?}: title, tags, and locked are managed by mkb and \
