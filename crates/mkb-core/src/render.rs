...
//! wiki structure visible (the whole point of mkb):
//!
//! - `[[target]]` → a Markdown link `[label](mkb:<id>)`; `mkb-view` styles it as a wikilink
//!   chip. Dangling targets link to `mkb:?unresolved`.
...
/// A plain `[[target]]` reference → a Markdown link in the shared `mkb:` scheme.
///
...
fn render_reference(vault: &Vault, target: &str, label: &str) -> String {
    match vault.resolve(target) {
        Some(id) => {
            ...
            format!("[{}](mkb:{id})", escape_link_text(&text))
        }
        None => format!("[{}](mkb:?unresolved)", escape_link_text(label)),
    }
}
...
fn resolve_embed(
    ...
    let id = match vault.resolve(target) {
        Some(id) => id,
        None => return format!("[⚠ {label}](mkb:?unresolved) *(unresolved embed)*"),
    };
    if visited.contains(&id) {
        // Cycle: link back to the offending block + a visible note, never recurse.
        return format!("[↻ {label}](mkb:{id}) *(transclusion cycle)*");
    }
    ...
    let header = format!("⧉ [{}](mkb:{id})", escape_link_text(&block.display_title()));
...
/// from a vault block). Unlike [`render_block`], which produces `mkb:`-linked embed *cards*,
...
/// `.md` file has nowhere to link a `mkb:` scheme). Total: cycles and dangling targets become
...
/// alias) as **plain text** — a standalone `.md` doc can't resolve the `mkb:` scheme. When a
...
fn dissolve_embed(
    ...
    let id = match vault.resolve(target) {
        Some(id) => id,
        None => return format!("<!-- mkb: unresolved embed: {target} -->"),
    };
    if visited.contains(&id) {
        return format!("<!-- mkb: transclusion cycle at {label} -->");
    }
    let block = match vault.block(&id) {
        Some(b) => b,
        None => return format!("<!-- mkb: unresolved embed: {target} -->"),
...
mod tests {
    ...
    #[test]
    fn reference_renders_mkb_link() {
        let (v, parent, child) = vault();
        let out = render_block(&v, &parent).unwrap();
        assert!(out.contains(&format!("(mkb:{child})")), "got: {out}");
    }
    ...
    fn bare_id_reference_uses_block_title_as_label() {
        ...
        assert_eq!(out, format!("see [Deployment Guide](mkb:{target}) now\n"));
    }
    ...
    fn explicit_alias_is_preserved() {
        ...
        let out = render_block(&v, &src).unwrap();
        assert!(
            out.contains(&format!("[the docs](mkb:{target})")),
            "got: {out}"
        );
    }
    ...
    fn cycle_is_broken_with_a_note() {
        ...
        assert!(out.contains("](mkb:"), "cycle note should link: {out}");
    }
    ...
    fn flat_dissolves_embeds_inline_no_card() {
        ...
        // Child content is inlined, with no embed-card blockquote and no mkb: links.
        assert!(out.contains("child content"), "got: {out}");
        assert!(!out.contains("> ⧉"), "should not be a card: {out}");
        assert!(!out.contains("mkb:"), "no mkb links in flat output: {out}");
        assert!(
            !out.contains("![["),
    ...
    fn flat_cycle_and_dangling_become_comments() {
        ...
        assert!(out.contains("<!-- mkb: transclusion cycle"), "got: {out}");

        let mut v2 = Vault::new();
        let c = BlockId::generate();
        v2.insert_source(c.clone(), "x\n\n![[01JJJJJJJJJJJJJJJJJJJJJJJJ]]\n\ny\n");
        let out2 = render_flat(&v2, &c).unwrap();
        assert!(out2.contains("<!-- mkb: unresolved embed"), "got: {out2}");
    }
    ...
    fn flat_inline_single_line_embed_splices_mid_sentence() {
        ...
        v.insert_source(
            host.clone(),
            &format!("mkb is ![[{tag}]] for developers.\n"),
        );
        let out = render_flat(&v, &host).unwrap();
        assert_eq!(out, "mkb is the fast knowledge base for developers.\n");
    }
