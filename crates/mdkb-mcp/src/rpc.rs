...
//! [`mdkb_protocol::Client`], so the MCP server stays a thin client of the one shared
//! `Service`.

use mdkb_protocol::Client;
    ...
    match method {
        "initialize" => Outcome::Reply(ok(
            ...
            json!({
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "mdkb-mcp", "version": env!("CARGO_PKG_VERSION")}
            }),
        )),
...
mod tests {
    ...
    fn client() -> Client {
        ...
        Client::new("/nonexistent/mdkb-test.sock")
    }
    ...
    fn initialize_returns_protocol_and_capabilities() {
        ...
        match handle_message(&client(), &msg) {
            Outcome::Reply(r) => {
                ...
                assert_eq!(r["result"]["serverInfo"]["name"], "mdkb-mcp");
            }
