...
//! vault's own `.mkb/config.json` (which configures the daemon's embedder). Keeping the
...
pub enum ConnectionConfig {
    ...
    Remote {
        /// `host:port` of the remote `mkbd --listen`.
        host: String,
    ...
    pub fn load() -> Registry {
        ...
        match std::fs::read_to_string(&path) {
            Ok(text) => serde_json::from_str(&text).unwrap_or_else(|e| {
                eprintln!(
                    "mkb: ignoring malformed {}: {e}; using the built-in default vault",
                    path.display()
                );
                Registry::builtin()
            }),
    ...
    pub fn default_connection(&self) -> ConnectionConfig {
        match &self.default {
            Some(name) => match self.vaults.iter().find(|e| &e.name == name) {
                ...
                None => {
                    eprintln!(
                        "mkb: default vault {name:?} is not in the registry; using the built-in \
                         default vault"
                    );
...
/// segment is the product name `mkb` on every platform (it is no longer the desktop app's bundle
/// id). Override with `$MKB_CONFIG_DIR` (e.g. point it at a synced folder).
fn client_config_dir() -> PathBuf {
    ...
    #[cfg(target_os = "macos")]
    {
        if let Some(home) = std::env::var_os("HOME") {
            return PathBuf::from(home).join("Library/Application Support/mkb");
        }
    }
    #[cfg(target_os = "windows")]
    {
        if let Some(appdata) = std::env::var_os("APPDATA") {
            return PathBuf::from(appdata).join("mkb");
        }
    }
    // Linux / fallback: XDG.
    if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
        return PathBuf::from(xdg).join("mkb");
    }
    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".config/mkb");
    }
    PathBuf::from(".mkb-config")
}
...
/// [`EnvSnapshot::read`], which treats an **empty** variable as absent (so `MKB_TOKEN=""` doesn't
...
pub fn resolve_client(inputs: &ClientInputs, mkbd_path: Option<&Path>) -> Result<Client, String> {
    ...
    connect_resolved(target, mkbd_path)
}
...
/// spawning one (detached) if none is reachable. `mkbd_path` lets a bundled binary be used
/// (e.g. the one shipped inside the desktop app); when `None`, the daemon is looked up beside
/// the current executable, then on `PATH`.
pub fn connect(cfg: &ConnectionConfig, mkbd_path: Option<&Path>) -> Result<Client, String> {
    match cfg {
        ConnectionConfig::Remote { host, token } => {
...
/// Ensure a daemon is reachable on `paths.socket`, spawning a **detached** `mkbd` if needed.
pub fn ensure_daemon(paths: &DaemonPaths, mkbd_path: Option<&Path>) -> Result<Client, String> {
    ...
    spawn_detached(paths, mkbd_path)?;
...
/// at the edge of the default. `MKB_READY_TIMEOUT_SECS` lets such environments (e.g. the export
...
/// Spawn `mkbd` for `paths`, fully detached so it outlives the spawning process.
fn spawn_detached(paths: &DaemonPaths, mkbd_path: Option<&Path>) -> Result<(), String> {
    let exe = resolve_mkbd(mkbd_path)?;
    // Daemon output goes to a log file in the vault's local `.mkb` dir.
    let log_path = paths.mkb_dir().join("mkbd.log");
...
/// rely on `PATH`.
fn resolve_mkbd(explicit: Option<&Path>) -> Result<PathBuf, String> {
    if let Some(p) = explicit {
        if p.exists() {
            return Ok(p.to_path_buf());
        }
        return Err(format!("configured mkbd path not found: {}", p.display()));
    }
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join(mkbd_filename());
            if candidate.exists() {
                return Ok(candidate);
            }
        }
    }
    Ok(PathBuf::from("mkbd"))
}

fn mkbd_filename() -> &'static str {
    if cfg!(windows) {
        "mkbd.exe"
    } else {
        "mkbd"
...
mod tests {
    ...
    fn load_save_round_trip() {
        ...
        let dir = std::env::temp_dir().join(format!("mkb-conncfg-{}", std::process::id()));
