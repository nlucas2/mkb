//! The mkb wire protocol: request/response types, a blocking client, and the shared
//! request dispatcher.
//!
//! Both ends speak newline-delimited JSON over a transport (a local socket or TCP). The
//! [`dispatch`] function maps a [`Request`] onto a [`mkb_core::Service`] and is the single
...
impl Client {
    ...
    ///
    /// - `MKB_REMOTE=host:port` (+ `MKB_TOKEN`) → a **remote TCP** client (token required).
    /// - else `MKB_SOCKET=/path` → that Unix socket.
    /// - else the local socket for `MKB_VAULT` (or the default vault).
    ///
    ...
    pub fn from_env() -> Result<Client, String> {
        if let Some(remote) = std::env::var_os("MKB_REMOTE") {
            let remote = remote.to_string_lossy().trim().to_string();
            if !remote.is_empty() {
                let token = std::env::var("MKB_TOKEN").unwrap_or_default();
                if token.is_empty() {
                    return Err(
                        "MKB_REMOTE is set but MKB_TOKEN is empty; a remote daemon requires a token"
                            .to_string(),
                    );
                }
        ...
        if let Some(sock) = std::env::var_os("MKB_SOCKET") {
...
mod tests {
    use super::*;
    use mkb_core::SyncEngine;
    use mkb_index::SqliteIndex;
    ...
    fn mutates_classifies_writes_and_reads() {
        let id = mkb_core::BlockId::generate();
    ...
    fn from_env_resolves_remote_socket_and_default() {
        std::env::remove_var("MKB_REMOTE");
        std::env::remove_var("MKB_SOCKET");
