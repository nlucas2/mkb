...
fn local_name(path: &Path) -> io::Result<Name<'static>> {
    #[cfg(unix)]
    {
        ...
        // The socket lives at the literal filesystem path under `<vault>/.mkb/`.
    ...
    #[cfg(windows)]
    {
        ...
        format!("mkb_{:016x}", hasher.finish()).to_ns_name::<GenericNamespaced>()
    }
}
...
mod tests {
    ...
    fn local_name_is_derivable() {
        ...
        assert!(local_name(Path::new("/tmp/v/.mkb/mkbd.sock")).is_ok());
    }

    #[test]
    fn bind_connect_roundtrip() {
        use std::io::{BufRead, BufReader, Write};
        let dir = std::env::temp_dir().join(format!("mkb-tp-{}", std::process::id()));
