...
use mkb_protocol::{resolve_client, ClientInputs};
use rpc::{handle_message, Outcome};

/// MCP server CLI. Pin it to a specific vault (`--vault`, or `$MKB_VAULT`) so an agent only ever
...
#[command(
    name = "mkb-mcp",
    version,
    about = "MCP server for mkb (stdio) — a thin client that forwards tool calls to the daemon",
    long_about = "Speaks MCP over stdin/stdout and forwards tool calls to mkbd (auto-started for \
                  a local vault). Pin the vault with --vault (or $MKB_VAULT) so an agent only \
                  acts on that knowledge base; otherwise the configured registry default is used."
)]
struct Cli {
    /// Vault directory to serve (supports a leading `~`). Overrides $MKB_VAULT and the registry
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    eprintln!("mkb-mcp: connected to daemon on {}", client.endpoint());
