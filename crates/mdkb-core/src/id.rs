...
//! invisible, namespaced HTML comment — `<!-- mdkb:<ulid> -->` — via [`NativeIdCodec`].
...
/// The native mdkb codec: encodes ids as `<!-- mdkb:<ulid> -->`.
...
fn native_re() -> &'static Regex {
    ...
    RE.get_or_init(|| {
        Regex::new(r"<!--\s*mdkb:([0-9A-Za-z]{26})\s*-->").expect("valid native id regex")
    })
}

impl IdCodec for NativeIdCodec {
    fn encode(&self, id: &BlockId) -> String {
        format!("<!-- mdkb:{} -->", id.as_str())
    }
...
mod tests {
    ...
    fn encode_uses_invisible_html_comment() {
        ...
        assert_eq!(
            ...
            "<!-- mdkb:01ARZ3NDEKTSV4RRFFQ69G5FAV -->"
        );
    }
