...
//! `mdkb export vault --check` did as a CI shell step, but now part of `cargo test --workspace`,
...
//! regenerating. Run `mdkb export vault` and commit the regenerated docs.

use std::path::{Path, PathBuf};

use mdkb_core::export::{plan_exports, Manifest};
use mdkb_core::{read_block_files, Vault};

/// The workspace root, derived from this crate's compile-time manifest dir (`crates/mdkb-core`).
...
fn generated_docs_match_their_source_blocks() {
    ...
    assert!(
        ...
        "generated docs are out of date with their source blocks: {drifted:?}\n\
         edit the block in vault/, then run `mdkb export vault` and commit the result.",
    );
}
