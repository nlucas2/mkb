...
//! that answers [`mkb_protocol`] requests by dispatching to the shared
//! [`mkb_core::Service`]. The daemon binds a **local** Unix socket only — never the network
...
use mkb_core::{Service, SyncEngine};
...
fn run() -> Result<(), String> {
    ...
    let lock_path = cfg.paths.mkb_dir().join("mkbd.lock");
    ...
    if cfg.socket().exists() {
        let probe = mkb_protocol::Client::new(cfg.socket());
        if probe.ping() {
            return Err(format!(
    ...
    let source = mkb_embed::FileConfig::load(cfg.paths.mkb_dir()).embedder;
    eprintln!("mkbd: embedder source = {source:?}");
    let engine = SyncEngine::new(cfg.vault(), index).with_embedder(mkb_embed::from_source(&source));
    let mut service = Service::new(engine);

    // Initial reconcile so the index reflects the vault before serving.
    let ctx = mkb_core::RequestContext::local();
    let report = service.reconcile(&ctx).map_err(|e| e.to_string())?;
    eprintln!(
        "mkbd: initial reconcile — {} changed, {} removed",
        report.changed.len(),
        report.removed.len()
    );
