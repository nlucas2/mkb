...
//!
//! Connections speak newline-delimited JSON [`mkb_protocol::Request`]s. Each is answered by
//! locking the shared [`mkb_core::Service`] and calling the shared
//! [`mkb_protocol::dispatch`].
//!
//! The local socket is cross-platform via [`mkb_protocol::transport`] (a Unix-domain socket
...
use mkb_core::{Caller, RequestContext, Scope};
use mkb_protocol::{decode_request, dispatch, encode_response, transport, Request, Response};
...
fn spawn_idle_watchdog(activity: Activity, timeout: Duration, leases: Leases, socket: PathBuf) {
    ...
    thread::spawn(move || loop {
        ...
        if should_reap(activity.idle_for(), timeout, &leases) {
            eprintln!(
                "mkbd: idle for {:?} (>= {:?}) and no interactive lease; shutting down {}",
                activity.idle_for(),
                timeout,
                socket.display()
            );
...
pub fn serve(
    ...
    let activity = idle_timeout.map(|timeout| {
        ...
        eprintln!("mkbd: idle self-shutdown armed ({timeout:?})");
        activity
    });
    ...
    if let Some(net) = net {
        ...
        match TcpListener::bind(&addr) {
            Ok(listener) => {
                eprintln!("mkbd: network listener on {addr} (token auth required)");
                thread::spawn(move || serve_tcp(listener, net, svc, act, lz, gen));
            }
            Err(e) => eprintln!("mkbd: failed to bind network listener {addr}: {e}"),
        }
    }
    ...
    eprintln!("mkbd: listening on {}", socket.display());
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    ...
                    if let Err(e) = handle(
                        ...
                        eprintln!("mkbd: connection error: {e}");
                    }
                });
            }
            Err(e) => eprintln!("mkbd: accept error: {e}"),
        }
    }
    Ok(())
}
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    let writer = match stream.try_clone() {
                        Ok(w) => w,
                        Err(e) => {
                            eprintln!("mkbd: tcp clone error: {e}");
                            return;
                        }
                    };
                    ...
                    if let Err(e) = handle(
                        ...
                        eprintln!("mkbd: tcp connection error: {e}");
                    }
                });
            }
            Err(e) => eprintln!("mkbd: tcp accept error: {e}"),
        }
    }
}
...
fn handle(
    ...
    loop {
        ...
        writer.flush()?;
        if shutting_down {
            eprintln!("mkbd: shutdown requested; removing socket and exiting");
