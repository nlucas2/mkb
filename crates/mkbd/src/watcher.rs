...
pub fn spawn(vault: PathBuf, service: SharedService, generation: Generation) {
    thread::spawn(move || {
        if let Err(e) = run(vault, service, generation) {
            eprintln!("mkbd: watcher stopped: {e}");
        }
    });
}
    ...
    loop {
        ...
        match guard.reconcile(&ctx) {
            Ok(report) if !report.is_empty() => {
                ...
                eprintln!(
                    "mkbd: watcher reconcile — {} changed, {} removed (generation {gen})",
                    report.changed.len(),
                    report.removed.len()
                );
            }
            Ok(_) => {}
            Err(e) => eprintln!("mkbd: watcher reconcile error: {e}"),
        }
    }
}
