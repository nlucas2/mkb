...
//!
//! ```sh
//! cargo build --release -p mdkb-embed --features onnx --example footprint
...
use mdkb_core::cosine_similarity;
use mdkb_embed::{Embedder, FastEmbedder};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Load vendored local files (no network) — the only supported path. Point MDKB_MODEL_DIR
    // at a model directory (e.g. the int8 BGE-small export) to benchmark it.
    let dir = std::env::var("MDKB_MODEL_DIR").map_err(|_| {
        "set MDKB_MODEL_DIR to a model directory (ONNX + tokenizer files) to run this benchmark"
    })?;
    let load_start = Instant::now();
    println!("loading from MDKB_MODEL_DIR={dir}");
