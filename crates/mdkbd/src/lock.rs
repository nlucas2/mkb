...
impl VaultLock {
    /// Try to acquire the exclusive lock at `path` (e.g. `<vault>/.mdkb/mdkbd.lock`).
    ///
...
mod tests {
    ...
    fn second_acquire_is_blocked_until_first_is_dropped() {
        let dir = std::env::temp_dir().join(format!("mdkb-lock-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("mdkbd.lock");
