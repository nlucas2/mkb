//! SQLite implementation of [`mkb_core::Index`] for the file-per-block model.
//!
...
impl SqliteIndex {
    ...
    fn vector_hits(
        ...
        let mut scored: Vec<SearchHit> = rows
            ...
            .into_iter()
            .map(|(block, blob)| {
                let candidate = mkb_core::bytes_to_vector(&blob);
                let score = mkb_core::cosine_similarity(vector, &candidate) as f64;
                SearchHit {
                    block,
                    score,
                    lineage: None,
...
impl Index for SqliteIndex {
    ...
    fn search(&self, query: &SearchQuery) -> Result<Vec<SearchHit>, IndexError> {
        ...
        match (&query.text, &query.vector) {
            (Some(_), Some(vector)) => {
                ...
                let fused = mkb_core::reciprocal_rank_fusion(&kw_ids, &vec_ids, 60.0);
                Ok(fused
                    .into_iter()
                    .filter_map(|(id, score)| {
    ...
    fn set_embedding(
        ...
        let blob = mkb_core::vector_to_bytes(vector);
        self.conn
            .execute(
